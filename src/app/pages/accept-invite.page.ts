...
export class AcceptInvitePage implements OnInit, OnDestroy {
  ...
  /** Token stored so the Accept button handler can use it */
  ...
  ngOnInit(): void {
    ...
    this.store.select(selectInvitationsPreviewResult)
      ...
      .subscribe(result => {
        this.preview = result;
        // Show the preview card — user must click "Accept Invitation" to continue
        this.view.set('preview');
        this.cdr.markForCheck();
      });
  }

  /** Called when user clicks the "Accept Invitation" button on the preview card */
  ...
  private async continueAfterPreview(token: string, preview: InvitationPreview): Promise<void> {
    // Step 2 — ensure the *correct* user is authenticated
    const isLoggedIn = await this.authService.isAuthenticated();

    if (isLoggedIn) {
      // If a different user is signed in (e.g. the owner opened the worker's link),
      // sign them out so the invited worker can set their password and sign in.
  ...
  private doAccept(companyId: string, token: string): void {
    ...
    this.store.select(selectInvitationsAcceptResult)
      ...
      .subscribe(result => {
        ...
        // Reload the user so the new membership appears in the store
